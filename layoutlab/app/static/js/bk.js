...
// Event listeners for layout size
document.getElementById('layoutSizeNum').addEventListener('input', updateLayoutSize);
document.getElementById('layoutSizeRange').addEventListener('input', updateLayoutSize);
document.getElementById('layoutHeightNum').addEventListener('input', updateLayoutSizeHeight);
document.getElementById('layoutHeightRange').addEventListener('input', updateLayoutSizeHeight);
//document.getElementById('layoutWidth').addEventListener('input', updateLayoutSize);

document.getElementById('layoutHeight').addEventListener('input', updateLayoutSizeHeight);
document.getElementById('layoutWidth').addEventListener('input', updateLayoutSizeFromWidth);
...
function updateUnit() {
    ...
    // Get inputs
    const numberInput = document.getElementById('layoutSizeNum');
    const rangeInput = document.getElementById('layoutSizeRange');
    const hnumberInput = document.getElementById('layoutHeightNum');
    const hrangeInput = document.getElementById('layoutHeightRange');

    // Set min and max based on selected unit
    let min, max;
    ...
    rangeInput.min = min;
    rangeInput.max = max;
    hnumberInput.min = min;
    hnumberInput.max = max;
    hrangeInput.min = min;
    hrangeInput.max = max;

    // Adjust current values if out of range
    if (numberInput.value < min) numberInput.value = min;
    if (numberInput.value > max) numberInput.value = max;
    if (rangeInput.value < min) rangeInput.value = min;
    if (rangeInput.value > max) rangeInput.value = max;
    if (hnumberInput.value < min) hnumberInput.value = min;
    if (hnumberInput.value > max) hnumberInput.value = max;
    if (hrangeInput.value < min) hrangeInput.value = min;
    if (hrangeInput.value > max) hrangeInput.value = max;

    // Call updateLayoutSize to apply changes
    updateLayoutSize({ target: numberInput });
    updateLayoutSizeHeight({ target: hnumberInput });
...
// Layout Size
function updateLayoutSize(event) {
    const value = parseInt(event.target.value, 10);
    if (isNaN(value)) return;

    // Sync number and range inputs
    document.getElementById('layoutSizeNum').value = value;
    document.getElementById('layoutSizeRange').value = value;
    document.getElementById('layoutWidth').value = value;


    // Set container size with chosen unit
    document.querySelector('.container').style.width = `${value}${unit}`;

}


function updateLayoutSizeHeight(event) {
    const value = parseInt(event.target.value, 10);
    if (isNaN(value)) return;

    // Sync number and range inputs
    document.getElementById('layoutHeightNum').value = value;
    document.getElementById('layoutHeightRange').value = value;
    document.getElementById('layoutHeight').value = value;


    // Set container size with chosen unit
    document.querySelector('.container').style.height = `${value}${unit}`;

}

// Function to update layoutSizeNum and layoutSizeRange from layoutWidth
...
// Function to update layoutWidth from layoutSizeNum or layoutSizeRange
...
function updatePresetSize() {
    ...
    const layoutwidth = document.getElementById('layoutSizeNum'); 
    const layoutheight = document.getElementById('layoutHeightNum'); 
    ...
    if (value.startsWith('screen-')) {
        ...
        const aspheight = (9 / 16) * size;
        container.style.width = `${size}px`;
        container.style.height = aspheight;
        ...
        // Handle print sizes
        const [width, height] = value.replace('print-', '').split('-');
...
function getGridConfiguration() {
    ...
    const parentContainerSettings = {
        ...
        layoutSize: document.getElementById('layoutSize').value || 100,
...
function loadGridConfiguration(configuration) {
    ...
    document.getElementById('layoutSize').value = parentContainerSettings.layoutSize || 100,
...
function resetInputsToDefault() {
    // Layout size
    const layoutSizeInput = document.getElementById('layoutSize');
    ...
    // Layout size inputs
    layoutSizeInput.value = layoutSizeInput.defaultValue;
...
document.addEventListener('DOMContentLoaded', updateLayoutSizeFromWidth);
document.addEventListener('DOMContentLoaded', updateLayoutSizeFromHeight);
